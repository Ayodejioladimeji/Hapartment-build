...
const DetailsScreen = ({ route }) => {
  ...
  // Chat with agents directly on whatsapp

  const openWhatsapp = () => {
    Linking.openURL(
      `http://api.whatsapp.com/send?phone=234
        ${postedBy.verification[0].identity_mobile}&text=""`
    );
  };
  ...
  return (
    <View style={styles.detailsWrapper}>
      ...
      >
        ...
        {/* Property Rented section */}
        {id === user._id && !acquired && (
          <View style={styles.rentWrapper}>
            <Text style={styles.rentHeading}>
              Click the button if this property have been rented
            </Text>
            <TouchableOpacity
              activeOpacity={0.7}
              onPress={handleAcquired}
              style={styles.rentBox}
            >
              {acquiredloading ? (
                <ActivityIndicator color={colors.white} size="small" />
              ) : (
                <Text style={styles.rentText}>Property Rented</Text>
              )}
            </TouchableOpacity>
          </View>
        )}
